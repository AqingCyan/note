...
- 从流程图可以看到，当一个用户第一次弹窗被分配到坐席后，就不会再更改了，如果他不再进行外呼，那么在坐席资源紧张的时候，无疑是一种浪费。
- 外呼中心与业务系统交互也与维护SDK交互，这并不合理，外呼中心理应只为外呼能力服务，即就是请求应该全在外呼SDK内部进行。
- 同一个账号只会分配一个坐席，用户多开页面都启用外呼的时候，会相互抢占坐席号，造成外呼的不稳定。
...
同时，由于只接入了一家外呼服务，存在一些局限性，首先是没有灾备措施，其次是号码资源也有限。因此需要接入更多的外呼服务商，覆盖到更多的资源。

原本的外呼是和业务系统高度耦合的，外呼SDK是被动的传入了业务方从外呼中心反复请求拿到的配置进行外呼的。并且业务方系统的是以弹窗出现为契机获取配置，
我们希望是在每一通电话实际动作的时候才获取配置，甚至可以做到每一通电话的外呼服务商不一样，坐席不一样，这样可以制定资源策略，提高外呼接通率。

因此，我们希望进行一次大的迭代，达到这样的目标：

- 在原有的第三方的代码包上做一层封装，设计一个较好的模式，方便后续的其他第三方的外呼代码接入。
- 与外呼中心的请求全在封装的SDK中处理，而业务方只需要调用方法外呼，挂断即可。当前外呼是否可以拨打，是否有号码资源是SDK和外呼中心自己交互决定，业务系统只能从SDK暴露的信息知道结果。
- 在每一通电话开始实际动作的时候才获取配置，一次性拿到所有的外呼配置后进行外呼。
- 可以在每一通开始的时候根据配置决定使用什么服务商的SDK和坐席，能做到灵活切换。
- 对同一坐席不同页面外呼，能够相互验证。

## 外呼2.0

如此一来，我们可以将现有的第三方外呼SDK做一个高度封装，里面只包含两个重要请求：获取呼叫配置，通话行为发送。

- 获取呼叫配置：从外呼中心拿到本次外呼使用哪一第三方的外呼能力，锁定返回坐席号和其他必要的信息（例如主叫号码，使用外呼策略：认证号、当地手机号）。
- 支持指定：业务方可以参数指定想使用什么外呼模式，禁拨名单是否强制拨打，指定呼叫模式无资源是否采用默认呼叫模式等等，来告知外呼中心最终生成的外呼策略。
- 坐席验证：在一次外呼开始的时候能告知呼叫中心该坐席的状态是在呼叫中，在别的页面再次外呼，同一坐席给予警告。正常挂断电话能告知外呼中心该坐席已经结束通话。

于是，我们可以梳理出以下的流程：

![新外呼流程](../assets/20210814new-webcall-sdk/awesome-webcall-process.png)

而我们基于第三方外呼的SDK架构应该如此：

![新外呼架构](../assets/20210814new-webcall-sdk/awesome-webcall-constructor.png)
